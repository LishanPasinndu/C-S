 ...
 * @author MSI
 */
public class vehistock1 extends javax.swing.JFrame {
    ...
    public void loadstock() {
        ...
        try {
            ResultSet rs = MySQL.search(" SELECT * FROM `vehistock` INNER JOIN `product` ON `product`.`id`=`vehistock`.`product` INNER JOIN `ref` ON `ref`.`id`=`vehistock`.`ref` INNER JOIN `company` ON `company`.`id`=`ref`.`company` INNER JOIN `vehicle` ON `vehicle`.`id`=`vehistock`.`vehicle`  WHERE `vehistock`.`status`='1' AND `company`.`name` LIKE '%Cycle%' ;");
    ...
    public void searchstock() {
        ...
        try {
            ...
            if (company.equals("Select")) {
                ...
                queryVector.add("`vehicle`.`number` = '" + company + "' AND `vehistock`.`status`='1' AND `company`.`name` LIKE '%Cycle%'  ");
            }
            //company
            ...
            if (pname.isEmpty()) {
                ...
                queryVector.add(" `product`.`pname` LIKE '%" + pname + "%' AND `vehistock`.`status`='1' AND `company`.`name` LIKE '%Cycle%' ");
            }
            //product
            ...
            if (ref.equals("Select")) {
                ...
                queryVector.add(" `ref`.`fname` = '" + ref + "' AND `vehistock`.`status`='1' AND `company`.`name` LIKE '%Cycle%' ");
            }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(vehistock1.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new vehistock1().setVisible(true);
            }
        });
    }
